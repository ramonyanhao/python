...
@app.route('/', methods=['GET', 'POST'])  # methods是响应request请求的HTTP方法，不是指定客户端访问的HTTP方法、
# 指定客户端访问的HTTP方法是在HTML文件上设置的，这里的HTTP方法是在log_analyze.py文件里的session函数中指定了POST.
# 通过浏览器访问默认为get方法，如果chrome浏览器使用POST访问，需要安装浏览器插件postman
def home():
...
204 No Content	请求已经被成功处理，但是没有返回新文档。浏览器应该继续显示原来的文档。如果用户定期地刷新页面，而Servlet可以确定用户文档足够新，
    这个状态代码是很有用的。
...
305 Use Proxy	客户请求的文档应该通过Location头所指明的代理服务器提取。(你不能直接访问网站，要通过某个代理才能进去。
...
4xx: 客户端错误
消息：	描述：
400 Bad Request	因为语法错误，服务器未能理解请求。(说明服务器无法理解用户的请求，除非进行修改，不然你按再多刷新也没有用。
    很有可能的情况是，你不小心输入错误了，导致服务器根本不知道你要表达什么。)

401 Unauthorized	合法请求，但对被请求页面的访问被禁止。因为被请求的页面需要身份验证，客户端没有提供或者身份验证失败。
402 Payment Required	此代码尚无法使用。
403 Forbidden	合法请求，但对被请求页面的访问被禁止。(出现403是因为服务器拒绝了你的地址请求，很有可能是你根本就没权限访问网站，
    就算你提供了身份验证也没用。讲真，很有可能是你被禁止访问了。)
    
404 Not Found	服务器无法找到被请求的页面。(404其实在日常生活中很常见了。代码的意思是找不到要查询的页面。非常有可能是网页被删除了。)
405 Method Not Allowed	请求中指定的方法不被允许。(资源被禁止，有可能是文件目录权限不够导致的。这个时候其实，只要赋予“完全控制”的权限，也是可以解决的)
406 Not Acceptable	服务器生成的响应无法被客户端所接受。
407 Proxy Authentication Required	用户必须首先使用代理服务器进行验证，这样请求才会被处理。
408 Request Timeout	请求超出了服务器的等待时间。(遇到408意味着你的请求发送到该网站花的时间比该网站的服务器准备等待的时间要长)
